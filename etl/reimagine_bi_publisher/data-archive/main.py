...
def run(known_args, pipeline_args):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        (
            p   
            | f"ListFiles" >> fileio.MatchFiles(data_file_prefix)
            | f'Reshuffle' >> beam.Reshuffle()
            
            # File Archival
            | f"ArchiveFiles" >> beam.ParDo(MoveFiles(), src_bucket, archive_bucket, archive_folder)
            
            # Logging total number of files being archived
            | "CountFiles" >> beam.combiners.Count.Globally()
